      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(220px, 1fr))', gap:12 }}>
        {filtered.map(c => (
          <div key={c.name} onClick={() => setSelectedCountry(c)}
            ...
            <div style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'var(--text3)', marginBottom:8 }}>{c.region}</div>
            <div style={{ marginBottom:8 }}><Tag status={c.status} small /></div>
            <div style={{ fontFamily:'var(--font)', fontSize:11, color:'var(--text2)', lineHeight:1.5 }}>
              {(c.regs || []).slice(0, 2).map((r, i) => <div key={i}>· {r}</div>)}
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
          <div style={{ gridColumn:'1/-1', padding:40, textAlign:'center', color:'var(--text3)', fontFamily:'var(--font)' }}>
          ...
          <>
            ...
            <div style={{ marginBottom:20 }}>
              ...
              {(selectedCountry.regs || []).map((r, i) => (
                <div key={i} style={{ fontFamily:'var(--font)', padding:'8px 0', borderBottom:'1px solid var(--surface2)', fontSize:13 }}>· {r}</div>
              ))}
            </div>
            <div style={{ marginBottom:24 }}>
              <SectionLabel style={{ marginBottom:8 }}>Context</SectionLabel>
              <p style={{ fontFamily:'var(--font)', fontSize:13, color:'var(--text2)', lineHeight:1.7 }}>
                {selectedCountry.notes || 'Regulatory data for this jurisdiction is being compiled.'}
              </p>
            </div>
