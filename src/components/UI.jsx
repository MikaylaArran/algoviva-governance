...
export function Badge({ severity }) {
  return <span style={{ fontFamily:'var(--font-mono)', fontSize:9, letterSpacing:1, padding:'2px 6px', color:'white', background: BADGE_BG[severity] || 'var(--text3)', marginRight:6 }}>{severity}</span>;
}

// ── SECTION LABEL ──
export function SectionLabel({ children, style }) {
  return <div style={{ fontFamily:'var(--font-mono)', fontSize:10, fontWeight:500, letterSpacing:2, textTransform:'uppercase', color:'var(--text3)', marginBottom:6, ...style }}>{children}</div>;
}
...
export function FilterBtn({ active, onClick, children }) {
  return (
    <button onClick={onClick} style={{ padding:'7px 14px', background: active ? 'var(--ink)' : 'white', border: active ? '1.5px solid var(--ink)' : '1.5px solid var(--border)', color: active ? 'white' : 'var(--text)', fontFamily:'var(--font)', fontSize:12, fontWeight:500, cursor:'pointer', transition:'all 0.15s' }}>
      {children}
    </button>
  );
}
  ...
  return (
    <div style={{ background:'var(--ink)', color:'white', padding:'20px 24px', marginTop:20, display:'flex', justifyContent:'space-between', alignItems:'center', gap:20, flexWrap:'wrap' }}>
      ...
      <div style={{ textAlign:'right', flexShrink:0 }}>
        <a href={`mailto:advisory@algoviva.com?subject=${subject}`} style={{ color:'#34d399', textDecoration:'none', fontWeight:600, fontSize:13, display:'block', marginBottom:4, fontFamily:'var(--font)' }}>
...
export function CiteLink({ citeRef, citeText, onOpen }) {
  return (
    <button onClick={() => onOpen(citeRef, citeText)} style={{ color:'var(--accent2)', fontFamily:'var(--font-mono)', fontSize:11, cursor:'pointer', borderBottom:'1px dashed var(--accent2)', background:'none', borderTop:'none', borderLeft:'none', borderRight:'none', padding:0, lineHeight:1.4 }}>
      {citeRef}
    </button>
  );
}